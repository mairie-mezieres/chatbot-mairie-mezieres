...
async function sendWeatherPush(vigilance, opts = {}) {
  ...
  // Mode test (opts.test) : message explicitement « test » — vérifier la réception
  // sans alarmer les habitants avec une fausse vigilance.
  const payload = JSON.stringify(opts.test ? {
    title: '🧪 Test alerte météo — MAT',
    body:  'Si vous voyez ceci, vos alertes météo fonctionnent ✅',
    icon:  './icon-192.png',
  ...
  console.log(`🌩️ Météo push${opts.test ? ' (TEST)' : ` (vigilance ${color}, level≥${level})`} → ${sent}/${subs.length} filtrés / ${allSubs.length} total (${dead.length} expirés purgés)`);
  await recordPushHistory({ type: opts.test ? 'meteo-test' : 'meteo', title: opts.test ? 'Test alerte météo' : `Vigilance ${color} — ${phenom}`, sent, total: subs.length, dead: dead.length });
...
// ── Identité d'une alerte (pour la déduplication durable) ────
// Une alerte est « la même » tant que son niveau, son phénomène ET son heure de
// fin ne changent pas. On exclut volontairement l'heure de DÉBUT : quand une
// alerte devient active, Météo-France fait avancer le début (les créneaux passés
// disparaissent), ce qui ne doit pas être vu comme un changement. L'heure de fin,
// elle, est stable et porte le sens « jusqu'à quand ». Résultat : une alerte
// inchangée n'est postée qu'une fois ; un vrai changement (escalade orange→rouge,
// nouveau phénomène, fin repoussée) produit une nouvelle signature → nouveau post.
function weatherAlertSignature(v) {
  if (!v) return "";
  return [Number(v.level), Number(v.phenomenon_id), v.end || ""].join("|");
}

// ── Verrous anti-doublon (atomiques) ─────────────────────────
// Empêchent deux vérifications concurrentes (cron interne /15 min, cron externe
// cron-job.org, ou chevauchement de redéploiement Render) de notifier deux fois
// la même alerte. La réservation NX est atomique côté Redis : un seul process
// gagne, les autres voient « déjà réservé » et s'abstiennent. Repli « fail-open »
// si Redis est indisponible : mieux vaut un doublon rare qu'une alerte de sécurité
// non envoyée.
//
// Push ET Facebook fonctionnent désormais à l'IDENTIQUE : une seule notification
// par alerte distincte (signature niveau+phénomène+fin). Les habitants ne sont pas
// re-notifiés tant que l'alerte ne change pas réellement — ils sont déjà au courant.
// Ces verrous ne servent qu'à éviter la course (TTL court) ; la déduplication
// DURABLE se fait par comparaison de signature avec mat:weather:last (Facebook) et
// mat:weather:last:push (push), dans routes/meteo.js et admin-email.js. Verrous
// séparés car une alerte peut être poussée (niveau ≥ 2) sans être postée (niveau ≥ 3).
const WEATHER_RACE_TTL_S = 2 * 3600;

async function _claim(key, ttl, force) {
  if (force) {
    await redisSetex(key, ttl, { at: new Date().toISOString() });
    return true;
  }
  const claim = await redisSetNxEx(key, { at: new Date().toISOString() }, ttl);
  // true = réservation acquise → agir ; false = déjà réservée → ignorer (doublon) ;
  // null = Redis indéterminé → agir (fail-open).
  return claim !== false;
}
async function _release(key) { try { await redisDel(key); } catch (_) {} }

function _pushClaimKey(v) { return `mat:weather:pushclaim:${weatherAlertSignature(v)}`; }
function _fbClaimKey(v)   { return `mat:weather:fbclaim:${weatherAlertSignature(v)}`; }

const claimWeatherPush           = (v, force) => _claim(_pushClaimKey(v), WEATHER_RACE_TTL_S, force);
const releaseWeatherPushClaim    = (v)        => _release(_pushClaimKey(v));
const claimWeatherFacebookPost   = (v, force) => _claim(_fbClaimKey(v), WEATHER_RACE_TTL_S, force);
const releaseWeatherFacebookPost = (v)        => _release(_fbClaimKey(v));
...
module.exports = {
  ...
  claimWeatherFacebookPost,
  releaseWeatherFacebookPost,
};
