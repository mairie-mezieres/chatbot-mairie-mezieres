...
// cron-job.org, ou chevauchement de redéploiement Render) de doubler une alerte.
// La réservation NX est atomique côté Redis : un seul process gagne, les autres
// voient « déjà réservé » et s'abstiennent. Repli « fail-open » si Redis est
// indisponible : mieux vaut un doublon rare qu'une alerte de sécurité non envoyée.
//
// Deux verrous SÉPARÉS, de portées différentes :
//   • push : clé niveau+phénomène, TTL 12h → rappelle l'alerte au plus toutes les
//     12h (utile en sécurité pendant un épisode long type canicule).
//   • Facebook : clé = SIGNATURE (niveau+phénomène+fin), TTL court → ne sert qu'à
//     éviter la course ; la déduplication durable du post FB se fait par comparaison
//     de signature avec mat:weather:last (cf. routes/meteo.js), donc une alerte
//     inchangée n'est jamais re-postée, contrairement au push.
const WEATHER_PUSH_CLAIM_TTL_S = 12 * 3600;
const WEATHER_FB_RACE_TTL_S    = 2 * 3600;
...
function _pushClaimKey(v) { return `mat:weather:pushclaim:${Number(v.level)}:${Number(v.phenomenon_id)}`; }
function _fbClaimKey(v)   { return `mat:weather:fbclaim:${weatherAlertSignature(v)}`; }

const claimWeatherPush           = (v, force) => _claim(_pushClaimKey(v), WEATHER_PUSH_CLAIM_TTL_S, force);
const releaseWeatherPushClaim    = (v)        => _release(_pushClaimKey(v));
const claimWeatherFacebookPost   = (v, force) => _claim(_fbClaimKey(v), WEATHER_FB_RACE_TTL_S, force);
