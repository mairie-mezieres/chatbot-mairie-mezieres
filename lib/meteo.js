...
// cron-job.org, ou chevauchement de redéploiement Render où l'ancienne et la
// nouvelle instance tournent en même temps) de doubler la même alerte
// (niveau + phénomène) — pour le push ET pour le post Facebook. La réservation
// NX est atomique côté Redis : un seul process gagne, les autres voient « déjà
// réservé » et s'abstiennent. Remplace l'ancien « lire-puis-écrire » qui laissait
// une fenêtre de course pendant la durée de l'envoi.
// Repli « fail-open » si Redis est indisponible : mieux vaut un doublon rare
// qu'une alerte de sécurité non envoyée.
const WEATHER_CLAIM_TTL_S = 12 * 3600;
function _weatherClaimKey(kind, v) {
  return `mat:weather:${kind}claim:${Number(v.level)}:${Number(v.phenomenon_id)}`;
}
async function _claimWeather(kind, vigilance, force) {
  const key = _weatherClaimKey(kind, vigilance);
  if (force) {
    // Forçage admin : (re)pose la réservation et laisse passer.
    await redisSetex(key, WEATHER_CLAIM_TTL_S, { at: new Date().toISOString() });
    return true;
  }
  const claim = await redisSetNxEx(key, { at: new Date().toISOString() }, WEATHER_CLAIM_TTL_S);
  // true = réservation acquise → agir ; false = déjà réservée → ignorer (doublon) ;
  // null = Redis indéterminé → agir (fail-open).
  return claim !== false;
}
async function _releaseWeather(kind, vigilance) {
  try { await redisDel(_weatherClaimKey(kind, vigilance)); } catch (_) {}
}

// Push (notifications) et post Facebook ont des verrous SÉPARÉS : une alerte
// peut très bien être poussée sans être (re)postée et inversement.
const claimWeatherPush         = (v, force) => _claimWeather("push", v, force);
const releaseWeatherPushClaim  = (v)        => _releaseWeather("push", v);
const claimWeatherFacebookPost = (v, force) => _claimWeather("fb", v, force);
const releaseWeatherFacebookPost = (v)      => _releaseWeather("fb", v);
