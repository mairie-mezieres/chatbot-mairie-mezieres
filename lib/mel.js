...
// Fiches « démarches administratives » VÉRIFIÉES (règles nationales stables —
// source : service-public.fr — avec renvois locaux), injectées dans le contexte
// pour le topic `demarches`. MEL répondait « je n'ai pas cette information »
// (ex. inscription listes électorales) faute de contexte : on lui DONNE les
// faits plutôt que de relâcher les garde-fous anti-hallucination.
// Pour ajouter une fiche : un objet { titre, texte } ici + les mots-clés
// correspondants dans KEYWORDS.demarches.
const DEMARCHES = [
  {
    titre: "Inscription sur les listes électorales",
    texte: "Conditions : avoir 18 ans, être de nationalité française (les citoyens de l'UE résidant en France peuvent s'inscrire pour les élections municipales et européennes) et avoir une attache avec la commune (domicile, résidence ou qualité de contribuable). Les jeunes de 18 ans sont inscrits automatiquement s'ils ont fait leur recensement citoyen à 16 ans dans la commune. Comment : en ligne via le téléservice https://www.service-public.gouv.fr/particuliers/vosdroits/R16396, ou à la mairie de Mézières-lez-Cléry (formulaire + pièce d'identité + justificatif de domicile de moins de 3 mois), ou par courrier. Quand : toute l'année ; pour voter à un scrutin donné, s'inscrire au plus tard le 6e vendredi précédant le 1er tour. Vérifier son inscription : téléservice « Interroger sa situation électorale » sur service-public.gouv.fr."
  },
  {
    titre: "Vote par procuration",
    texte: "Demande en ligne sur maprocuration.gouv.fr (validation d'identité ensuite en gendarmerie ou commissariat, ou via l'application France Identité), ou directement par formulaire en gendarmerie/commissariat. Le mandataire vote dans le bureau de vote du mandant. Faire la démarche le plus tôt possible avant le scrutin."
  },
  {
    titre: "Recensement citoyen (16 ans)",
    texte: "Obligatoire pour les jeunes Français dans les 3 mois suivant le 16e anniversaire : se présenter à la mairie de Mézières-lez-Cléry avec pièce d'identité et livret de famille. L'attestation de recensement est demandée pour les examens (bac, permis) ; le recensement déclenche la convocation à la Journée Défense et Citoyenneté puis l'inscription automatique sur les listes électorales à 18 ans."
  },
  {
    titre: "Carte nationale d'identité et passeport",
    texte: "Ces titres ne se font PAS à la mairie de Mézières-lez-Cléry : il faut un rendez-vous dans une mairie équipée d'une station biométrique — la plus proche pour les habitants est généralement Cléry-Saint-André. Gagner du temps : pré-demande en ligne sur ants.gouv.fr, puis se présenter au rendez-vous avec le numéro de pré-demande et les justificatifs."
  },
  {
    titre: "Actes d'état civil (naissance, mariage, décès)",
    texte: "La copie ou l'extrait d'acte se demande à la mairie de la commune où l'événement a eu lieu (guichet, courrier ou en ligne via service-public.fr). Pour un événement survenu à Mézières-lez-Cléry, s'adresser à la mairie. Démarche gratuite."
  },
  {
    titre: "PACS",
    texte: "Le PACS s'enregistre à la mairie de la commune de résidence commune (sur rendez-vous) ou chez un notaire. Pièces principales : convention de PACS, déclaration conjointe (Cerfa), actes de naissance et pièces d'identité."
  },
];

function demarchesContext() {
  return "=== DÉMARCHES ADMINISTRATIVES (fiches vérifiées — règles nationales service-public.fr) ===\n"
    + DEMARCHES.map(d => `• ${d.titre} : ${d.texte}`).join("\n")
    + "\nPour un cas particulier, orienter vers la mairie (02 38 45 61 76, mairie@mezieres-lez-clery.fr) ou service-public.fr.";
}
...
const KEYWORDS = {
  ...
  demarches:      ["carte identité","passeport","naissance","mariage","décès","état civil","acte","certificat","demarche","démarche","élection","election","électoral","electoral","liste électorale","voter","procuration","bureau de vote","carte électorale","carte electorale","recensement","jdc","journée défense","pacs","cni"],
...
async function buildContext(userText, explicitTopic = null) {
  ...
  for (const topic of topics) {
    ...
    // Démarches : fiches vérifiées (élections, recensement, CNI, état civil, PACS)
    // EN PLUS des pages du site communal récupérées par le bloc SOURCES.
    if (topic === "demarches") parts.push(demarchesContext());
    if (topic === "transport") {
      if (remiNeedsRefresh()) {
...
module.exports = {
  ...
  DEMARCHES, demarchesContext, detectTopics
};
