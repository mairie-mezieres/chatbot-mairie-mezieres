...
async function sendSignalCommentPush(token, commentText, tagId) {
  if (!token) return { skipped: true, reason: "no token" };
  const preview = (commentText || "").substring(0, 120);
  return sendPushToToken(token, {
    title: "💬 Réponse de la mairie",
    body: preview || "La mairie a commenté votre signalement.",
    icon: "./icon-192.png",
    badge: "./icon-badge.png",
    tag: `signal-comment-${tagId}`,
    data: { url: "./#signalements", open: "signalements" }
  });
}

async function sendBugCommentPush(token, commentText, tagId) {
  if (!token) return { skipped: true, reason: "no token" };
  const preview = (commentText || "").substring(0, 120);
  return sendPushToToken(token, {
    title: "💬 Réponse de la mairie",
    body: preview || "La mairie a commenté votre rapport de bug.",
    icon: "./icon-192.png",
    badge: "./icon-badge.png",
    tag: `bug-comment-${tagId}`,
    data: { url: "./#bugs", open: "bugs" }
  });
}
...
module.exports = {
  ...
  sendSignalCommentPush, sendBugCommentPush
};
