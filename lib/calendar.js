...
function getGoogleCalendarClient() {
  ...
  try {
    ...
    // ⚠️ Forme OBJET obligatoire, pas la forme positionnelle.
    //
    // `new google.auth.JWT(email, null, key, scopes)` fonctionne encore en
    // googleapis 134, mais à partir des versions récentes (vérifié en 174.0.1)
    // les arguments positionnels sont IGNORÉS EN SILENCE : le constructeur ne
    // lève rien et renvoie un client dont `email`, `key` et `scopes` valent
    // `undefined`. Le `catch` ci-dessous ne se déclenche donc pas, et la panne
    // n'apparaît qu'au premier appel réel — au moment où la mairie publie un
    // événement, sans le moindre indice au démarrage.
    //
    // La forme objet est correcte dans les deux versions : elle nous protège
    // de la montée à venir sans rien changer au comportement actuel.
    const auth = new google.auth.JWT({
      email: credentials.client_email,
      key: credentials.private_key,
      scopes: ["https://www.googleapis.com/auth/calendar"],
    });
    // Garde-fou : si une montée future déplaçait à nouveau le contrat, on veut
    // le savoir ici, à l'initialisation, et pas au premier événement publié.
    if (!auth.email || !auth.key || !auth.scopes) {
      console.warn(
        "Google Calendar init fail: le client JWT n'a pas retenu ses identifiants " +
        "(email/key/scopes). Contrat de google.auth.JWT probablement modifié."
      );
      return null;
    }
