...
// Lecture « parlante » : distingue une clé ABSENTE d'un Redis INJOIGNABLE.
//   { ok: true,  value: null }  → la clé n'existe pas
//   { ok: true,  value: … }     → la clé existe
//   { ok: false, value: null }  → réseau, timeout, 429, JSON illisible…
//
// ⛔ `redisGet` ne peut pas faire cette différence : il renvoie `null` dans les
// deux cas. Pour un cache, c'est sans conséquence ; pour une donnée qu'on
// RÉÉCRIT ENSUITE, c'est destructeur — `readStats()` faisait
// `(await redisGet("mat:stats")) || {}`, si bien qu'un timeout au démarrage
// (payload de plusieurs Mo, timeout 8 s) repartait d'un objet vide et le flush
// suivant écrasait tout l'historique cinq minutes plus tard, sans un mot.
// Toute lecture suivie d'une réécriture doit passer par ici. Voir ADR-0017.
async function redisGetResult(key, options = {}) {
  if (!REDIS_URL) return { ok: true, value: null };
  try {
    const r = await axios.get(
      `${REDIS_URL}/get/${encodeURIComponent(key)}`,
      { headers: { Authorization: `Bearer ${REDIS_TOKEN}` }, timeout: options.timeout || 8000 }
    );
...
async function redisGet(key) {
  const { value } = await redisGetResult(key);
  return value;
}

// Renvoie true si l'écriture a été acquittée par Upstash, false sinon.
// ⚠️ L'échec reste silencieux côté appelant (on n'a jamais fait dépendre une
// réponse HTTP d'une écriture Redis) : c'est à l'appelant qui REMPLACE une
// donnée de tester ce booléen avant de considérer l'ancienne comme périmée.
...
// SET atomique « si absent » avec expiration (verrou / dédup anti-course).
// Renvoie true si la clé a été posée (réservation acquise), false si elle
// existait déjà (réservation refusée), null si Redis est indisponible ou en
// erreur (l'appelant décide alors de son comportement de repli).
async function redisSetNxEx(key, value, ttlSeconds) {
  if (!REDIS_URL || _isRedis429()) return null;
  try {
    const res = await redisPipeline([["SET", key, JSON.stringify(value), "EX", String(ttlSeconds), "NX"]]);
    const r = res && res[0];
    if (!r || r.error) return null;
    return r.result === "OK";
  } catch (e) {
    console.warn(`Redis SET NX ${key}:`, e.message);
    return null;
  }
}
...
module.exports = {
  redisGet, redisGetResult, redisSet, redisSetex, redisSetNxEx, redisDel, redisPipeline, redisLRange,
