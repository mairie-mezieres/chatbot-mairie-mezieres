...
async function fetchVigieauStatus(insee) {
  // ⚠️ 45204 = Mézières-lez-Cléry ; 45203 est Meung-sur-Loire.
  const commune = insee || VIGIEAU_COMMUNE_INSEE || "45204";
  const attempts = [];
  if (Number.isFinite(VIGIEAU_LAT) && Number.isFinite(VIGIEAU_LON)) {
    attempts.push({ path: "coordonnees", params: { lon: VIGIEAU_LON, lat: VIGIEAU_LAT, commune, profil: "particulier" } });
  }
  attempts.push({ path: "commune", params: { commune, profil: "particulier" } });

  const results = await Promise.all(attempts.map((a) => _queryZonesStatus(a.params)));
  // Journal par requête : sans lui, `complete: false` ne dit ni LAQUELLE des deux
  // a échoué ni POURQUOI — et une lecture partielle bloque durablement toute
  // publication de baisse (decideDroughtAction, règle 3). C'est la seule trace.
  const diag = results.map((r, i) => ({
    path: attempts[i].path,
    ok: r.level != null,
    level: r.level != null ? r.level : null,
    reason: r.level != null ? null : (r.reason || "inconnu"),
    detail: r.level != null || r.detail == null ? null : String(r.detail),
  }));
  const usable = results.filter((s) => s.level != null);
  // Aucune réponse exploitable : on remonte l'échec de la requête prioritaire.
  if (usable.length === 0) return Object.assign({}, results[0], { attempts: diag });
...
// Échec(s) d'une lecture partielle, en clair — pour le diagnostic admin et les logs.
// Retourne null si la lecture est complète (ou si on n'a pas le détail par requête).
const _REASON_FR = {
  network: "API injoignable",
  "multi-zone": "409 multi-zones",
  shape: "réponse inattendue",
  unknown: "raison inconnue",
};
const _PATH_FR = { coordonnees: "par coordonnées", commune: "par commune" };

function partialReadReason(status) {
  const list = Array.isArray(status && status.attempts) ? status.attempts.filter((a) => !a.ok) : [];
  if (!list.length) return null;
  return list
    .map((a) => {
      const why = a.reason === "http" ? `HTTP ${a.detail || "?"}` : (_REASON_FR[a.reason] || a.reason || "raison inconnue");
      return `requête ${_PATH_FR[a.path] || a.path} en échec (${why})`;
    })
    .join(" ; ");
}
