...
async function readIdeaFilters() { const c=memGet("mat:idees:filtres"); if(c!==undefined)return c; const v=(await redisGet("mat:idees:filtres"))||[]; memSet("mat:idees:filtres",v,MEM_TTL_SHORT); return Array.isArray(v)?v:[]; }
async function writeIdeaFilters(d){ memSet("mat:idees:filtres",d,MEM_TTL_SHORT); await redisSet("mat:idees:filtres",d); }
...
async function readPluiDocs()  { const c=memGet("mat:docs:plui");   if(c!==undefined)return c; const v=(await redisGet("mat:docs:plui"))||[];  memSet("mat:docs:plui",v,MEM_TTL_SHORT);    return v; }
async function writePluiDocs(d){ memSet("mat:docs:plui",d,MEM_TTL_SHORT);    await redisSet("mat:docs:plui",d); }
...
// ⚠️ `GET /sondages` lit les résultats de CHAQUE sondage actif, et l'app
// appelle cette route à chaque ouverture : sans miroir mémoire, c'était une
// commande Redis par sondage et par visite — la seule dépense qui montait
// linéairement avec la fréquentation, avec `/carburant`.
const _sondageResultsKey = id => "mat:sondage:results:" + id;
async function readSondageResults(id) {
  const k = _sondageResultsKey(id);
  const c = memGet(k);
  if (c !== undefined) return c;
  const v = (await redisGet(k)) || { total: 0, answers: {} };
  memSet(k, v, MEM_TTL_SHORT);
  return v;
}
async function writeSondageResults(id,d) { memSet(_sondageResultsKey(id), d, MEM_TTL_SHORT); await redisSet(_sondageResultsKey(id), d); }
function forgetSondageResults(id)        { memDel(_sondageResultsKey(id)); }
...
// La persistance vit dans `lib/stats-store.js` : deux clés Redis séparées par
// leur rythme de changement (socle historique / tranche du jour), au lieu d'un
// JSON unique réécrit intégralement toutes les 5 minutes. Voir ADR-0017.
// Les fonctions gardent leur signature : tout le code appelant est inchangé.
const _stats = require("./stats-store");
const STATS_FLUSH_MS = 5 * 60 * 1000; // flush toutes les 5 minutes

const readStats     = _stats.readStats;
const writeStats    = async (d) => _stats.writeStats(d);
const readIaStats   = _stats.readIaStats;
const writeIaStats  = async (d) => _stats.writeIaStats(d);
const flushStatsNow = _stats.flushStatsNow;
...
module.exports = {
  ...
  readIdeaFilters, writeIdeaFilters,
  ...
  readPluiDocs, writePluiDocs,
  readSondages, writeSondages,
  readPhotos, writePhotos,
  readSondageResults, writeSondageResults, forgetSondageResults,
