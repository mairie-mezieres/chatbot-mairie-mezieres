...
| **Prix carburant** (`lib/carburant.js`, `routes/carburant.js`) — ⚠️ `maj` est une chaîne d'affichage **sans année**, seul `majISO` (horodatage brut) se compare d'une station à l'autre ; ⛔ **la clé Redis suit la forme du payload** (`mat:carburant:v9`) : ajouter un champ sans changer la clé sert l'ancien format pendant une heure ; ⛔ **chaque carburant porte SA date** (`sp95Maj*`, `gazoleMaj*`) — le Leclerc de Tavers servait un SP95 du 08/09 sous un gazole du 16/09, et garder la première date trouvée datait les deux du 08/09 (l'ordre des champs inversé, c'est un prix périmé annoncé « du jour ») ; ⛔ `maj`/`majISO` au niveau station = le **plus ancien** des relevés (valeur sûre pour qui afficherait tout sous une seule date) — ⚠️ l'app, elle, **n'affiche que le relevé le plus récent** de chaque station et recalcule la date sur ce qu'elle montre (ADR-0047) ; ⛔ **deux stations sur un même code postal** (Olivet) → `id` de jeu de données, et « à défaut le premier enregistrement » ne vaut que s'il n'y en a qu'un — sinon `null`, jamais les prix du voisin | `app-mezieres/docs/adr/0047-un-carburant-a-sa-propre-date.md` puis `0033-un-prix-sans-sa-date-est-un-prix-du-jour.md` et `app-mezieres/docs/specifications-techniques/STD-07-services-pratiques.md` |
