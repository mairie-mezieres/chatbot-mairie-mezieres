...
| **Alertes sécheresse VigiEau** (séparées de la vigilance météo) — `lib/vigieau.js`, `routes/eau.js`. ⚠️ Une **lecture partielle** (`complete: false`) bloque **toute publication de baisse** (`descent-incomplete`) : la fin des restrictions ne serait jamais annoncée, et le seul signe est un message du diagnostic qui se lit comme un aléa passager. Le champ `attempts` dit **laquelle** des deux requêtes est tombée et **pourquoi** — un `409 multi-zones` par commune est permanent, un `HTTP 5xx` non | `GUIDE-ADMIN.md` §5ter puis `docs/adr/0016-lecture-partielle-vigieau-dire-laquelle-et-pourquoi.md` |
...
| **Kit réplication « Partager »** — profils de communes, mail quotidien, `POST /stats/partager`. ⚠️ les **essais** (« ville test », « Cancale », tout nom contenant le mot *test*/*essai*/*demo*) ne sont **ni enregistrés ni restitués** — filtre `lib/partager.js`, appliqué à l'écriture **et** à la lecture, extensible par `PARTAGER_IGNORE_COMMUNES` ; les compteurs `partager_visite`/`partager_prompt`, eux, n'ont pas de nom de commune et comptent toujours les essais | `GUIDE-ADMIN.md` §6bis |
...
| **Prix carburant** (`lib/carburant.js`, `routes/carburant.js`) — ⚠️ `maj` est une chaîne d'affichage **sans année**, seul `majISO` (horodatage brut) se compare d'une station à l'autre ; ⛔ **la clé Redis suit la forme du payload** (`mat:carburant:v11`) : ajouter un champ sans changer la clé sert l'ancien format pendant une heure — et après un correctif qui touche à la **désignation** des stations, la garder sert une heure de prix attribués à la mauvaise enseigne ; ⛔ **chaque carburant porte SA date** (`sp95Maj*`, `gazoleMaj*`) — le Leclerc de Tavers servait un SP95 du 08/09 sous un gazole du 16/09, et garder la première date trouvée datait les deux du 08/09 (l'ordre des champs inversé, c'est un prix périmé annoncé « du jour ») ; ⛔ `maj`/`majISO` au niveau station = le **plus ancien** des relevés (valeur sûre pour qui afficherait tout sous une seule date) — ⚠️ l'app, elle, **n'affiche que le relevé le plus récent** de chaque station et recalcule la date sur ce qu'elle montre (ADR-0047) ; ⛔ **le flux instantané v2 ne porte AUCUNE enseigne** (un `id`, un `cp`, une `adresse`, une `ville`, des prix) : la correspondance par **marque ne matche jamais**, c'est `liste[0]` qui a désigné les stations depuis toujours — l'avoir restreint a vidé **trois cartes sur six** en production, tests verts, parce que ceux-ci fabriquaient un champ `ensigne` qui n'existe pas ; le repli n'est donc refusé que si **deux de NOS stations** partagent le code postal ; ⛔ **un `id` est une donnée, pas une déduction** — il se relève sur `prix-carburants.gouv.fr/station/<id>`, et **seul** l'`id` départage deux stations d'un même code postal (le garde-fou protège d'un mauvais choix par marque, **pas** d'un id recopié de travers) ; ⛔ **le 45160 en porte deux** — `45160005` E.Leclerc Olivet, `45160006` relais du Coudray — et c'est en les renseignant qu'on a découvert que `liste[0]` y désignait le **relais** : l'app affichait ses prix sous le nom du Leclerc **depuis l'origine**, sans rien pour le trahir | `app-mezieres/docs/adr/0047-un-carburant-a-sa-propre-date.md` puis `0033-un-prix-sans-sa-date-est-un-prix-du-jour.md` et `app-mezieres/docs/specifications-techniques/STD-07-services-pratiques.md` |
...
| **Plusieurs photos par actualité** (`lib/actu.js`, `routes/admin-actus.js`) — ⛔ `photos[]` **s'ajoute** à `photo` / `photoPublicId` (la **couverture**) : le push, la vignette bureau et la carte « prochaine manifestation » lisent `photo` seul, et les actus d'avant la v4.109 n'ont que lui → **toute** lecture des images passe par `actuPhotoList`, l'enjeu étant les **trois chemins de suppression** (actu, purge, annulation d'une programmation) : un `publicId` oublié reste sur Cloudinary sous un `ok: true` ; ⚠️ N ≥ 2 → photos en `published=false` puis **un** `POST /feed` avec `attached_media` (l'ambiguïté timeout/5xx ne porte donc plus que sur cette dernière requête) ; ⛔ `fallbackUsed` reste **faux** quand le post a des photos malgré un refus partiel (`skippedPhotos`) | `app-mezieres/docs/adr/0039-plusieurs-photos-une-couverture-et-un-balayage.md` puis `GUIDE-ADMIN.md` §5quater et `test/actu-photos.test.js` |
| **Webhook Facebook → actu à plusieurs photos** (sens ENTRANT, `routes/webhook.js`, `lib/facebook.js`) — ⛔ `full_picture` ne rend que la **couverture**, et `change.value.photo` (une chaîne) est **absent** d'un post multi-photos, qui porte `change.value.photos` (un tableau) : cinq images sur six disparaissaient sous une actu d'apparence normale ; les images vivent dans `attachments{subattachments{media}}` ; ⛔ les deux sources (corps du webhook / Graph API) ne se **fusionnent JAMAIS** — la même photo n'y a pas la même URL (deux CDN, deux signatures), donc chaque image partirait en double sans qu'aucun dédoublonnage par URL ne le voie : on garde celle qui en décrit le **plus**, la Graph API l'emportant à égalité ; ⚠️ sans `PAGE_ACCESS_TOKEN` la Graph API rend `[]` **sans erreur**, d'où le repli ; ⚠️ **pas de rollback global** sur un échec Cloudinary, contrairement à `routes/admin-actus.js` — Facebook ne renvoie pas l'événement, refuser l'actu c'est perdre le post | `app-mezieres/docs/adr/0046-facebook-vers-l-app-deux-sources-jamais-fusionnees.md` puis `GUIDE-ADMIN.md` §5 et `test/webhook-photos.test.js` |
| **Suivi du dépôt** (`.github/workflows/suivi-depot.yml` + `scripts/suivi-depot.js`) — vérification et traitement des PR et issues ouvertes : ⛔ **plus une PR est « à nous », plus on agit** (automatique → commentaire si conflit ou CI rouge, relance unique ; **humaine → aucun commentaire, jamais**) ; ⛔ **aucune fermeture, aucun push** (`contents: read`) — fermer la PR d'un robot, c'est perdre la mise à jour qu'il proposait ; ⛔ le canal par défaut est le **résumé du run** ; ⚠️ l'issue `liens-morts` est **écartée** (son workflow la referme lui-même) ; ⚠️ **jumeau dans `app-mezieres`** — toute correction de fond se reporte dans les deux | `app-mezieres/docs/adr/0043-suivi-du-depot-agir-selon-qui-tient-la-pr.md` |
| **Fusion automatique Dependabot** (`.github/workflows/dependabot-auto-merge.yml` + `scripts/dependabot-auto-merge.js`) — ⛔ **jamais une majeure, et jamais une mineure en 0.x** (en pré-1.0 c'est la mineure qui porte les ruptures, semver §4 — `@anthropic-ai/sdk 0.124 → 0.125` n'est donc PAS fusionnée) ; ⛔ **« aucune » n'est pas « verte »** : sans check ayant tourné, on ne fusionne pas ; ⛔ la PR doit ne toucher **que** `package.json`/`package-lock.json`, et l'auteur est lu **sur l'API** ; ⚠️ **pas d'`--auto` natif** — `main` n'étant pas protégée, l'auto-merge GitHub fusionnerait **sans attendre la CI** ; ⚠️ le socle `scripts/lib/gh.js` porte la lecture de CI, **commune** avec le suivi (deux lectures divergentes porteraient sur la décision la plus lourde) ; ⛔ **`null == false` est VRAI** dans une expression Actions (les deux opérandes sont castés en nombre) — une `inputs.*` comparée à `false` vaut donc « coché » hors `workflow_dispatch`, ce qui a désactivé les mineures **dans le seul mode qui compte** et reste **invisible à l'essai** — verrouillé par `scripts/check-workflow-inputs.js` | `app-mezieres/docs/adr/0045-fusionner-une-mise-a-jour-de-version-sans-relecture.md` |
| **Trafic de test dans les statistiques** (`isSyntheticTraffic`, `lib/stats.js` → `POST /stats/track`) — ⛔ **la CI n'est pas un habitant** : six specs Playwright de `app-mezieres` ont appelé cette route pour de vrai du 31 août au 17 septembre 2026, avec un `deviceId` neuf par test (profil vierge) → jusqu'à **~550 « visiteurs uniques » par jour** et « Carburant » à 234 ouvertures ; ⛔ **le critère est l'ORIGINE, pas le `User-Agent`** — Playwright pose un UA de vrai navigateur via `devices['Pixel 7']`, « HeadlessChrome » n'y apparaît pas ; ce qui ne ment pas, c'est `127.0.0.1:4173` ; ⚠️ la route répond **200** sans rien écrire (un 4xx ferait diagnostiquer une panne) ; ⛔ **un filtre trop large efface de vrais habitants sans laisser de trace** — origine illisible ou absente = on compte, et `test/stats-trafic-synthetique.test.js` le verrouille ; ⚠️ **l'historique de cette période est pollué et ne se nettoie pas** | `app-mezieres/docs/adr/0048-la-ci-n-est-pas-un-habitant.md` |
