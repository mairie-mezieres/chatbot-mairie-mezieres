...
| **Prix carburant** (`lib/carburant.js`, `routes/carburant.js`) — ⚠️ `maj` est une chaîne d'affichage **sans année**, seul `majISO` (horodatage brut) se compare d'une station à l'autre ; ⛔ **la clé Redis suit la forme du payload** (`mat:carburant:v10`) : ajouter un champ sans changer la clé sert l'ancien format pendant une heure ; ⛔ **chaque carburant porte SA date** (`sp95Maj*`, `gazoleMaj*`) — le Leclerc de Tavers servait un SP95 du 08/09 sous un gazole du 16/09, et garder la première date trouvée datait les deux du 08/09 (l'ordre des champs inversé, c'est un prix périmé annoncé « du jour ») ; ⛔ `maj`/`majISO` au niveau station = le **plus ancien** des relevés (valeur sûre pour qui afficherait tout sous une seule date) — ⚠️ l'app, elle, **n'affiche que le relevé le plus récent** de chaque station et recalcule la date sur ce qu'elle montre (ADR-0047) ; ⛔ **le flux instantané v2 ne porte AUCUNE enseigne** (un `id`, un `cp`, une `adresse`, une `ville`, des prix) : la correspondance par **marque ne matche jamais**, c'est `liste[0]` qui désigne les cinq stations depuis toujours — l'avoir restreint a vidé **trois cartes sur six** en production, tests verts, parce que ceux-ci fabriquaient un champ `ensigne` qui n'existe pas ; le repli n'est donc refusé que si **deux de NOS stations** partagent le code postal ; ⛔ **un `id` est une donnée, pas une déduction** — `45160006`, relevé sur un titre de résultat de recherche, a affiché les prix du Leclerc d'Olivet sous le nom du relais du Coudray (le garde-fou protège d'un mauvais choix par marque, **pas** d'un id recopié de travers) : le Coudray attend **les deux** identifiants, relevés sur `prix-carburants.gouv.fr/station/<id>` | `app-mezieres/docs/adr/0047-un-carburant-a-sa-propre-date.md` puis `0033-un-prix-sans-sa-date-est-un-prix-du-jour.md` et `app-mezieres/docs/specifications-techniques/STD-07-services-pratiques.md` |
